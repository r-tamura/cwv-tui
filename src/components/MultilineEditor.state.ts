/**
 * Pure state model for the multiline editor. Lives in its own module so the
 * sibling component file can export only the React component — which is what
 * `react-doctor/only-export-components` (and the React Fast Refresh contract
 * it stands in for) wants.
 *
 * The text is a single string that may contain "\n"; the cursor is a
 * character offset into that string, with `cursor === text.length` meaning
